...
**Approved refinement, 2026-09-25 (D22):** Phase 6 status is a collection of configured enabled apps, with one app as the one-row case. This section's earlier selected-app-only presentation language is superseded. Every app remains independently accounted and enforced; no overall caps, shared grace, cross-app enforcement or passive unrelated-app tracking is authorized. Keep stable shared-brand tray artwork; one-app tooltips may name the app/remaining allowance, while multiple apps use counts and restricted counts. Compact rows expose read-only detail, bounded scrolling handles larger collections, and ordering places grace first, then restricted, running, available, lowest remaining allowance and stable alphabetical ties.
...
Hover examples: `ScreenTime — Apex Legends: 42 min remaining` or `ScreenTime — 3 apps monitored · 1 restricted`. Truncate safely to the actual framework tooltip limit. Keep updates on meaningful status/minute changes, not a per-second global refresh. During grace show its remaining time; distinguish starting, no configured apps, observed running/not observed, and unavailable allowance truthfully. Windows may place icons in its overflow area; users may pin it through Windows preferences.
...
### Phase 6 — Collection-based tray/status and protected configuration/Exit
...
- **Risk:** medium: accidental authorization bypass or stale/misleading status. Keep commands behind one access service; the D22 collection presentation never coordinates policy. Notification preferences remain presentation-only and must not contend with authoritative persistence or rebase accounting.
- **Validation:** automate read-only status, one/many app facts, wrong/right/cancelled protected commands, panel closure with continued enforcement, and native tray recreation handling where practical. Inspect actual panel placement/visuals and scaling. Run prior regression tests. Under the 2026-09-25 owner objective, a fresh Apex gate is required only when the independent reviewer finds actual changed rendering code/evidence justifies it; do not automatically repeat the full accepted Phase 5 game procedure. Unproven explicitly required physical gates still require owner evidence.
