...
## Package validation correction B2-V2

Independent package review `5322152585` retained one failure from candidate-a's four actual-package cases: the resource/protected-command case observed empty popup descendants immediately after the general window helper. That helper also requests foreground activation. Diagnostic copies retaining the helper plus a content wait passed 1/3; exact fixture-title/content reacquisition without activation or reopening passed 3/3. No definitive cause for the original stale/empty UIA state is claimed.

The package test now polls the exact fixture PID/title and required empty-state text without calling SetForeground or reopening the popup. It retains actionable title/HWND/PID/content diagnostics and every subsequent wrong-password, unchanged-policy, Settings, Exit and clean-exit assertion. This is test-only; production source is unchanged. Build passed, and the affected actual-package case passed **three consecutive runs** (`PackagePopupReadiness`, `-2`, `-3`) against a new extraction of the unchanged candidate-a archive, SHA-256 `45bfc778d92814192775a64aca69bbde1906575205bc73b0d25efcd7d54c6b60`. The original failure remains recorded. Candidate-a is preserved; new source provenance requires a new unique candidate/reproduction and independent exact-head/package review before release.
