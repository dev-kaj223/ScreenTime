...
Autostart is disabled; the existing TimeGuard Run entry is preserved. Launch manually. A fresh production profile defaults its configurable settings shortcut to `Ctrl+Alt+S`; saved production choices are honored. Development retains `Ctrl+Alt+Shift+S`. Tray access remains available if a shortcut conflicts. Policy/password/data changes remain local. First-run password success opens initial configuration without another password challenge. Left tray click opens/activates one read-only Dashboard; right click toggles a temporary interactive status popup with Dashboard/Settings/Exit actions. Click-away or Escape dismisses it. Settings and Exit remain protected; closing views never stops enforcement.
...
`Test-ReleasePackage.ps1` verifies every hash, absence of test dependencies, actual distribution defaults, Test override, rejection of legacy arguments, and native-load failure. The read-only probe runs before logger/storage/startup setup; successful and failed Test-profile probes must not create their requested directory. Default verification compares before/after real profile hashes and all current-user startup values. It verifies included runtime/native module paths with machine-runtime lookup disabled. All actual app smoke tests launch the extracted executable in disposable Test profiles. They exercise setup, resources/tray, duplicate launch, protected Settings/Exit, actual quota crossing to an original twenty-minute episode, relaunch denial, restart identity/deadline preservation and seeded near-expiry recovery/termination. No test-host MonitorService substitutes for packaged enforcement.

When an owner production ScreenTime instance is already running, its database can be locked and naturally changing. Do not stop it to satisfy a byte comparison. The optional paired `-ActiveProductionProcessId <known PID> -ActiveProductionStartUtcTicks <known creation ticks>` arguments require a matching live ScreenTime instance, retain its process handle, and verify its survival afterward. In this explicit mode only `%AppData%\ScreenTime` is excluded from byte comparison, reported as `ProductionProfileBytes: NotComparedActiveOwner`. TimeGuard and Development profiles, startup values, package inventory, runtime/native probes and isolated smoke tests remain strict. Incorrect or incomplete identity parameters fail; other inaccessible files are not silently skipped. Independently review the probe's pre-initialization boundary on the exact candidate source and retain the separate prior-release artifact hash check. This mode does not establish unchanged production database bytes or authorize profile access by normal app launches.
...
The owner-authorized target is `v0.1.0-beta.2`, an unsigned **draft prerelease only** after independent review and any reviewer-required owner UX gate. The app version is `0.1.0-beta.2`; SDK informational metadata also carries the source revision. Use new unique Beta 2 output labels and preserve the existing Beta 1 draft and exact archive unchanged. In addition to the package enforcement smoke, validate first-run configuration, singleton Dashboard, native right-tray toggle/click-away/Escape/footer, AM/PM multi-day editors and save/reload in disposable profiles. Do not launch a real production profile for automated tests or stop an owner's live application to simplify preservation checks.
