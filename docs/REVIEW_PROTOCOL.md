...
- The [ScreenTime validation workflow](../.github/workflows/screentime-validation.yml) builds and runs Core tests on `screentime-dev` PRs. It does not certify interactive UI or Apex acceptance. The package workflow targets `screentime-dev` and uploads unsigned candidates only; it creates no tags/public releases. A PR with no checks is not a pass. Never bypass failed/missing required checks or required reviewers.
